...
int main(void)
{
    ...
    for (i=0; i<500; i++)
    {
        ...
        s1 += sum.order / index;
        s2 += sum.setup / index;
        s3 += sum.holding / index;
        s4 += sum.shortage / index;

        Sleep(1100);
    }

    printf("Executing the modified sis2 %d times and taking the means of the outputs the results are,", i);
    printf("\nfor %ld time intervals, ", STOP);
