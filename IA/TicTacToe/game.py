...
class Game:
    ...
    def startGame(self):
        ...
        while not self.done:
            ...
            input()
            if self.player == 'X':
                self.computerMovement1()
            #elif self.player == 'O':
            #    self.computerMovement2()
    ...
    def computerMovement1(self):
        global firstMove
        if not firstMove:
            (line, column) = self.randomMove()
            firstMove = True
        ...
        afbeta = alfabeta()
        (line, column) = afbeta.searchMoveComputer1(self.board)
    ...
    def computerMovement2(self):
        global firstMove
        if not firstMove:
            (line, column) = self.randomMove()
            firstMove = True
            self.board[line][column] = 'O'
            self.drawX0(line, column, False)
            self.player = 'X'
            return

        afbeta = alfabeta()
        (line, column) = afbeta.searchMoveComputer2(self.board)
