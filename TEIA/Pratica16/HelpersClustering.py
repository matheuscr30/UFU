...
class SimplifiedSilhouette():
    ...
    def get_a(self, item, cluster):
        return minkowski_distance(item['data'], cluster['centroid'], 2)

    def get_b(self, item, item_cluster_id):
        b = 0

        for cluster_id, cluster in self.clusters.items():
            if cluster_id == item_cluster_id:
                continue
            b += minkowski_distance(item['data'], cluster['centroid'], 2)

        b /= len(self.clusters) - 1
        return b
    ...
    def calculate(self):
        ...
        for cluster_id, cluster in self.clusters.items():
            for item in cluster['group']:
                total_items += 1
                a = self.get_a(item, cluster)
                b = self.get_b(item, cluster_id)
                s = (b - a) / max(a, b)
                silhouette += s

        silhouette /= total_items
        return silhouette
